...
bool TerrainShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	result = InitializeShader(device, hwnd, L"../Engine/terrain.vsh", L"../Engine/terrain.psh");
	if(!result)
	{
		return false;
	}
...
bool TerrainShaderClass::SetShaderParameters(ID3D11DeviceContext* deviceContext, D3DXMATRIX worldMatrix, 
	D3DXMATRIX viewMatrix, D3DXMATRIX projectionMatrix, D3DXVECTOR4 ambientColor, D3DXVECTOR4 diffuseColor, 
	D3DXVECTOR3 lightDirection, ID3D11ShaderResourceView* texture)
{
	HRESULT result;
