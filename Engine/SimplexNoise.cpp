...
float SimplexNoise::noise2D(float xin, float yin)
{
	...
	float F2 = 0.5f*(float)(sqrt(3.0)-1.0);
	float s = (xin+yin)*F2;
	// Hairy factor for 2D
	int i = fastfloor(xin+s);
	int j = fastfloor(yin+s);
	float G2 = (float)(3.0-sqrt(3.0f))/6.0f;
	float t = (i+j)*G2;
	float X0 = i-t;
