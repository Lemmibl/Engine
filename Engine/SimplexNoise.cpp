...
float SimplexNoise::noise3D(float xin, float yin, float zin)
{ 
	float n0, n1, n2, n3;
	...
	// Determine which simplex we are in.
	int i1, j1, k1;
	// Ofsets for second corner of simplex in (i,j,k) coords
	int i2, j2, k2;
	// Ofsets for third corner of simplex in (i,j,k) coords
	if(x0>=y0) {
		if(y0>=z0)
	...
	// c = 1/6.
	// Ofsets for second corner in (x,y,z) coords
	float x1 = x0 - i1 + G3;
	float y1 = y0 - j1 + G3;
	float z1 = z0 - k1 + G3;

	// Ofsets for third corner in (x,y,z) coords
	float x2 = x0 - i2 + 2.0f*G3;
	float y2 = y0 - j2 + 2.0f*G3;
	float z2 = z0 - k2 + 2.0f*G3;

	// Ofsets for last corner in (x,y,z) coords
