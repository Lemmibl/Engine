...
float SimplexNoise::SimplexNoise3D(float xin, float yin, float zin) 
{
	...
	// c = 1/6.
	const float x1 = x0 - i1 + G3; // Ofsets for second corner in (x,y,z) coords
	const float y1 = y0 - j1 + G3;
	const float z1 = z0 - k1 + G3;
	const float x2 = x0 - i2 + G3TimesTwo; // Offsets for third corner in (x,y,z) coords
	const float y2 = y0 - j2 + G3TimesTwo;
	const float z2 = z0 - k2 + G3TimesTwo;
