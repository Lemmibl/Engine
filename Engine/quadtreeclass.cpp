...
bool QuadTreeClass::Initialize(TerrainClass* terrain, ID3D11Device* device)
{
	...
	CreateTreeNode(parentNode, centerX, centerZ, width, device);
...
void QuadTreeClass::Render(FrustumClass* frustum, ID3D11DeviceContext* deviceContext, TerrainShaderClass* shader)
{
	...
	RenderNode(parentNode, frustum, deviceContext, shader);
...
void QuadTreeClass::CalculateMeshDimensions(int vertexCount, float& centerX, float& centerZ, float& meshWidth)
{
	...
	minWidth = fabsf(vertexList[0].position.x - centerX);
	minDepth = fabsf(vertexList[0].position.z - centerZ);

	// Go through all the vertices and find the maximum and minimum width and depth of the mesh.
	for(i=0; i<vertexCount; i++)
	{
		width = fabsf(vertexList[i].position.x - centerX);	
		depth = fabsf(vertexList[i].position.z - centerZ);	
...
void QuadTreeClass::CreateTreeNode(NodeType* node, float positionX, float positionZ, float width, ID3D11Device* device)
{
	...
	for(i=0; i<triangleCount; i++)
	{
		...
		if(result == true)
		{
			...
			vertices[index].texture = vertexList[vertexIndex].texture;
...
bool QuadTreeClass::IsTriangleContained(int index, float positionX, float positionZ, float width)
{
	...
	// Get the three vertices of this triangle from the vertex list.
	x1 = vertexList[vertexIndex].position.x;
	z1 = vertexList[vertexIndex].position.z;
	vertexIndex++;

	x2 = vertexList[vertexIndex].position.x;
	z2 = vertexList[vertexIndex].position.z;
	vertexIndex++;

	x3 = vertexList[vertexIndex].position.x;
	z3 = vertexList[vertexIndex].position.z;
